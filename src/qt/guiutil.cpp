...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("uidd:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("uidd-%s.desktop", chain);
}
...
QString formatNiceTimeOffset(qint64 secs)
{
    ...
    const int YEAR_IN_SECONDS = 31695552; // Average length of year in Gregorian calendar
    if(secs < 60)
    {
